# -*- coding: utf-8 -*-
"""
Created on Sun Apr  3 15:07:38 2022
...
def affective_bias_finder(emotion, pair1_label, pair2_label):
    ...
    plot_name = 'intensity_plot_over_emo_'+str(model_name)+'_'+str(emotion_label_text)+'_'+str(pair1_label)+'_'+str(pair2_label)
    ...
    value, pvalue = ttest_rel(pair1_emotion_list, pair2_emotion_list)
    print("significance test values for "+ str( pair1_label) + " and " + str(pair2_label)+ " with " + str(emotion_label_text)+": ", value, pvalue)
    
    
    # Demographic Parity
    pair1_emotion_prediction_indices = np.array(np.where(sent_pair_1[:,2]== emotion))
    ...
    pair2_emotion_prediction_indices = np.array(np.where(sent_pair_2[:,2]== emotion))
    ...
    # Equal Opportunity
    pair1_gt_equal_pred = np.array(np.where((sent_pair_1[:,1]== emotion) & (sent_pair_1[:,1] == sent_pair_1[:,2])))
    pair1_gt_equal_pred_count = pair1_gt_equal_pred.shape[1]
    pair1_gt_emotion_count = np.array(np.where(sent_pair_1[:,1]== emotion)).shape[1]
    equal_opportunity_pair1 = pair1_gt_equal_pred_count/pair1_gt_emotion_count

    pair2_gt_equal_pred = np.array(np.where((sent_pair_2[:,1]== emotion) & (sent_pair_2[:,1] == sent_pair_2[:,2])))
    pair2_gt_equal_pred_count = pair2_gt_equal_pred.shape[1]
    pair2_gt_emotion_count = np.array(np.where(sent_pair_2[:,1]== emotion)).shape[1]
    equal_opportunity_pair2 = pair2_gt_equal_pred_count/pair2_gt_emotion_count
    print("Equal Opportunity over " +str(pair1_label)+ " vs " +str(pair2_label) +": ", equal_opportunity_pair1, '/', equal_opportunity_pair2)
...
# Violine Plot for confidence score    
    ...
    for i in range(len(sent_pair_1)):
        if (sent_pair_1[i][2] == sent_pair_2[i][2]) & (sent_pair_1[i][1] == sent_pair_2[i][1]) & (sent_pair_1[i][1] == sent_pair_1[i][2]) & (sent_pair_2[i][1] == sent_pair_2[i][2]):
            emo = sent_pair_1[i][1]
            score_pair_1 = np.float64(sent_pair_1[i][3])
            score_pair_2 = np.float64(sent_pair_2[i][3])
    ...
    plot_name = 'confidence_plot_over_emo_'+str(model_name)+'_'+str(pair1_label_text)+'_'+str(pair2_label_text)
    #plt.savefig(plot_path+plot_name+'.png')
    plt.show()
    return 
...
eval_corpus = 'eec'
context = 'gender'
model_name = 'BERT' #'T5', 'XLNet', 'GPT-2', 'BERT'
...
file1 = open('sentence_paires_predictions/gender/eec/eec_male_only_1400_eec_bert_predictions.csv')
file2 = open('sentence_paires_predictions/gender/eec/eec_female_only_1400_eec_bert_predictions.csv')
