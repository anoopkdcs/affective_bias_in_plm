# -*- coding: utf-8 -*-
"""
Created on Sun Apr 17 19:47:24 2022
...
def affective_bias_finder(data1, data2, pair1_label, pair2_label, model_name):
    ...
    plot_name = 'plots/'+str(context)+'/'+str(eval_corpus)+'/'+'intensity_plot_over_plm_noGt_'+str(model_name)+'_'+str(pair1_label)+'_'+str(pair2_label)
...
def confidence_score_finder(data1, data2, model_name):    
    ...
    for i in range(len(data1)):
        if (data1[i][1] == data2[i][1]):
            score_pair_1 = np.float64(data1[i][2])
            score_pair_2 = np.float64(data2[i][2])
...
def confi_score_violine_plot(cscore1, cscore2, cscore3, cscore4, pair1, pair2):
    ...
    plot_name = 'plots/'+str(context)+'/'+str(eval_corpus)+'/'+'confidence_plot_over_plm_noGt_'+str(context)+'_'+str(pair1_label)+'_'+str(pair2_label)
    #plt.savefig(plot_name+'.png')
    return
...
# --------- Main function ------
#Change Variables 
eval_corpus = 'csp' # only 'csp' no change 
context = 'religion'
pair1_label = 'Muslim'
pair2_label = 'Jew'
