# -*- coding: utf-8 -*-
"""
Created on Mon Apr 18 12:32:57 2022

@author: Intensity Plot for Male/Female/Non-binary accros plms and no groud truth 
for CSP Data
"""
import csv 
import numpy as np 
...
def affective_bias_finder(data1, data2, data3, pair1_label, pair2_label, pair3_label, model_name):
    ...
    for i in range(len(data1)):
        if (data1[i][1] == data2[i][1]) & (data1[i][1] == data3[i][1]) & (data2[i][1] == data3[i][1]):
            x_indices.append(i)
            emo_intensity_list1.append(np.float64(data1[i][2]))
            emo_intensity_list2.append(np.float64(data2[i][2]))
            emo_intensity_list3.append(np.float64(data3[i][2]))
    ...
    plot_name = 'plots/'+str(context)+'/'+str(eval_corpus)+'/'+'intensity_plot_over_plm_mjc_'+str(model_name)+'_'+str(pair1_label)+'_'+str(pair2_label)
...
bert_pair3_path = 'sentence_paires_predictions/religion/csp/csp_christian_104_csp_bert_predictions.csv'
...
gpt2_pair3_path = 'sentence_paires_predictions/religion/csp/csp_christian_104_csp_gpt2_predictions.csv'
...
xlnet_pair3_path = 'sentence_paires_predictions/religion/csp/csp_christian_104_csp_XLNet_predictions.csv'
...
t5_pair3_path = 'sentence_paires_predictions/religion/csp/csp_christian_104_csp_T5_predictions.csv'
